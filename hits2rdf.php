<?php
// hits2rdf.php          Norbert Haider, University of Vienna, 2011-2014
 ...
 *
 * Script exports hit reactions as RD files
 * 
 */
...
if (strlen($hits) > 0) {
  ...
  for ($i = 0; $i < $for_limit; $i++) {
    ...
    if (is_numeric($db_id) && ($db_id > 0)) {
      ...
      $qstr   = "SELECT struc FROM $rxnstructable WHERE rxn_id = $hit_id";
      $result = mysql_query($qstr)
        or die("Query failed! (hits2rdf #1)");    
      $rows  = mysql_num_rows($result);     // number of candidate structures
      if ($rows > 0) {
        while ($line = mysql_fetch_array($result, MYSQL_ASSOC)) {
          $struc   = $line["struc"];
          $struc = chop($struc);
          echo "\$RFMT \$RIREG $hit_id\n";
          echo "$struc\n";
        }  // while
      }  // if $rows > 0
    }
